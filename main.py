...
def TrackImages():
    ...
    # Replace this line with the URL of your IP webcam stream
    url=ip.get()
    webcam_url = url
    
    # Open the IP webcam stream
    cam = cv2.VideoCapture(webcam_url)
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                bb = str(aa)
                bb = bb[2:-2]
                criminal = [str(ID), '', bb, '', str(date), '', str(timeStamp)]
                if not image_saved:
                    ts = time.time()
                    date =datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
                    t=str(ts)
                    d=str(date)
                    cv2.imwrite("detected_suspects/suspect_" + d + "_" + t + ".jpg", im)  # Save the image
                    image_saved = True
            else:
                Id = 'Unknown'
                bb = str(Id)
            cv2.putText(im, str(bb), (x, y + h), font, 1, (255, 255, 255), 2)
        cv2.imshow('finding criminals', im)
        if (cv2.waitKey(1) == ord('q')):
            break
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    exists = os.path.isfile("crime\crime_" + date + ".csv")
    if exists:
        with open("crime\crime_" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
            writer.writerow(criminal)
        csvFile1.close()
    else:
        with open("crime\crime_" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
            ...
            writer.writerow(criminal)
        csvFile1.close()
    with open("crime\crime_" + date + ".csv", 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
...
message3 = tk.Label(window, text="suspect detection" ,fg="white",bg="#262523" ,width=55 ,height=1,font=('times', 29, ' bold '))
...
lbl3 = tk.Label(frame1, text="criminal",width=20  ,fg="black"  ,bg="#00aeff"  ,height=1 ,font=('times', 17, ' bold '))
...
takeImg = tk.Button(frame2, text="Take crimnal Images", command=TakeImages  ,fg="white"  ,bg="blue"  ,width=34  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
...
trackImg = tk.Button(frame1, text="find criminal from survilance", command=TrackImages  ,fg="black"  ,bg="yellow"  ,width=35  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
